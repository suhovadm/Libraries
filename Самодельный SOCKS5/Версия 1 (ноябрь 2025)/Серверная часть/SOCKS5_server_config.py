# 1. Главный конфигурационный файл SOCKS5 сервера,
# который содержит все настройки и параметры работы прокси-сервера.
...
# ----- ЗДЕСЬ НАЧИНАЕТ РАБОТУ ДЕКОРАТОР. -----
    ...
    # Удаляем настройку токена аутентификации
...
# ----- ЗДЕСЬ ЗАКАНЧИВАЕТ РАБОТУ ДЕКОРАТОР. -----
    ...
    # Эти подсети запрещены, чтобы клиенты не могли через мой SOCKS-прокси получить доступ
    # к внутренним сетям. Это на тот случай, если этим SOCKS5 буду пользоваться не только я.
    FORBIDDEN_NETWORKS = [
        ...
        '169.254.0.0/16', # link-local адреса (аварийные, служебные адреса,
        # предназначенные для автоматической коммуникации между устройствами,
        # находящимися в одном сегменте локальной сети, когда централизованная
        # конфигурация (например, от DHCP-сервера) недоступна.
        '224.0.0.0/4', # мультикаст (один источник, много получателей).
    ]
