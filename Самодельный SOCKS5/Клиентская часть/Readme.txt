...
5.1 Принимает SOCKS5 запрос от локального приложения,
5.2 Подключается к удалённому прокси-серверу,
5.3 Выполняет аутентификацию с прокси,
5.4 Шифрует и пересылает оригинальный SOCKS5 запрос,
5.5 Дешифрует и передаёт ответ обратно клиенту,
5.6 Запускает туннель для передачи данных.
