...
from server_config import ServerConfig # пользовательский конфиг файл с настройками безопасности.
...
class CryptoUtils:
    ...
    def __init__(self):
        ...
        # auth_token - токен для проверки подлинности:
        # простой SHA-256 хеш от ServerConfig.AUTH_TOKEN_SECRET
    ...
    def decrypt(self, encrypted_data: bytes) -> bytes:
        ...
        # Разделение компонентов.from
        iv = encrypted_data[:16] # IV: первые 16 байт.
        ciphertext = encrypted_data[16:-32] # ciphertext: данные между IV и HMAC (с 17-го байта до 32-го с конца)
        received_hmac = encrypted_data[-32:] # recieved_hmac: последние 32 байта.
    ...
    # Метод verify_token() - проверка токена.
    # Сравнивает переданный токен с сохранённым auth_token.
    # Использует hmac.compare_digest() для безопасного сравнения.
    # Защищено от атак по времени выполнения. За защиту отвечает функция hmac.compare_digest().
    # Почему именно эта функция?
    # Обычное сравнение строк/байт через == или token == self.auth_token уязвимо к timing-атакам, потому что:
    # сравнение прекращается при первом несовпадении байт,
    # время выполнения зависит от того, на каком байте произошло несовпадение,
    # атакующий может измерить время ответа и постепенно подобрать правильный токен.
    # hmac.compare_digest() защищена от таких атак потому что:
    # всегда сравнивает все байты до конца, независимо от несовпадений,
    # время выполнения постоянно и не зависит от содержания данных,
    # специально разработана для сравнения криптографических хешей и токенов.
    def verify_token(self, token: bytes) -> bool:
        """Проверка токена аутентификации"""
        return hmac.compare_digest(token, self.auth_token)

    # Пример уязвимого кода (уязвимого к timing-атакам):
    # def verify_token(token: bytes) -> bool:
    #   return token == self.auth_token # Прекращает сравнение при первом несовпадении.

    # Правильная реализация:
    # def verify_token(self, token: bytes) -> bool:
    #   return hmac.compare_digest(token, self.auth_token) # Сравнивает байты всегда.