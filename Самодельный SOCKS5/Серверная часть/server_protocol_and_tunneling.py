...
from server_config import ServerConfig # конфигурация сервера.

# Заводим класс AddressValidator - валдиатор адресов.
    ...
    def validate_address(self, addr: str, port: int) -> bool:
        try:
            ...
            try:
                ...
                # Цикл проходится по всем запрещённым подсетям и проверяет,
                # принадлежит ли IP адрес текущей запрещённой подсети.
...
class ProtocolHandler:
    ...
    def authenticate_client(self, client_sock: socket.socket) -> bool:
        ...
        try:
            client_sock.settimeout(5.0) # устанавливаем таймаут 5 секунд на операцию recv
    ...
    # Обрабатывает SOCKS5 handshake, возвращает сокет удалённого сервера или None.
    def handle_handshake(self, client_sock: socket.socket) -> Optional[socket.socket]:
        ...
        try:
            ...
            if not data or data[0] != 0x05:
                ...
                # Логиурем неверную версию SOCKS.
        ...
        def client_to_remote():
            ...
            while running[0]:
                try:
                    ...
                    # Дешифруем полученные даннеы и отправляем на удаленный хост
...
# Функция передачи данных от удалённого сервера к клиенту.
        ...
        t1.start() # запустить
        t2.start() # запустить
        # daemon=True означает, что потоки завершатся при завершении main-потока.
